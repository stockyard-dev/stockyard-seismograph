...
s.mux.HandleFunc("GET /api/events",s.list);s.mux.HandleFunc("POST /api/events",s.create);s.mux.HandleFunc("GET /api/events/{id}",s.get);s.mux.HandleFunc("DELETE /api/events/{id}",s.del)
...
func(s *Server)list(w http.ResponseWriter,r *http.Request){wj(w,200,map[string]any{"events":oe(s.db.List())})}
func(s *Server)create(w http.ResponseWriter,r *http.Request){var e store.Event;json.NewDecoder(r.Body).Decode(&e);if e.Type==""{we(w,400,"type required");return};s.db.Create(&e);wj(w,201,s.db.Get(e.ID))}
...
func(s *Server)stats(w http.ResponseWriter,r *http.Request){wj(w,200,map[string]int{"events":s.db.Count()})}
